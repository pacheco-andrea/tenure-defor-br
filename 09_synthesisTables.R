...
wd_plots <- paste0(wdmain, "plots/")
wd_sum <- paste0(wdmain, "model_summaries/")
setwd(wd_sum)

mypublic_table <- read.csv("all_models_vspublic_202012.csv")
myprivate_table <- read.csv("all_models_vsprivate_202012.csv")
mypublic_table_altPA <- read.csv("all_models_vspublic_altPA.csv")
myprivate_table_altPA <- read.csv("all_models_vsprivate_altPA.csv")
...
get_modelcounts <- function(control_table, treatment, scales){
  ...
  mydata$OrigEffect <- mydata$Effect
  mydata$Effect[which(mydata$p_value >= 0.05)] <- 0
  # only include in dataset models which were calculated
  mydata <- mydata[which(!is.na(mydata$p_value)),]
  
  # increase deforestation
  inc <- nrow(mydata[which(mydata$Effect > 0),]) 
  p.inc <- inc/nrow(mydata)
  # decrease deforestation
  dec <- nrow(mydata[which(mydata$Effect < 0),]) 
  p.dec <- dec/nrow(mydata)
  # how many nonsignificant?
  nonsig <- nrow(mydata[which(mydata$p_value >= 0.05),]) 
  ...
  b_inc <- sum(1-(mydata$ImbAfter[which(mydata$Effect > 0)])) # subtract 1-imbalance, so that it become a simple balance metric, and value reflects "validity" of effect
  ...
  b_dec <- sum(1-(mydata$ImbAfter[which(mydata$Effect < 0)]))
  p.b_dec <- b_dec/(sum(1-(mydata$ImbAfter)))
  
  # weighted nonsignificant
  b_nonsig <- sum(1-mydata$ImbAfter[which(mydata$p_value >= 0.05)]) # sum of "counts" of nonsig models weighted by balance
...
# test:
# control_table = mypublic_table[which(mypublic_table$Treatment != "Communal"),]
getMyCounts <- function(control_table,consistentScales){
  ...
  if(consistentScales==T){
    ...
    control_table$rm <- NA
    control_table$rm <- ifelse(is.na(control_table$p_value),T, F)# specify the if condition to be sensitive to p-values being NA - i.e. a model that wasn't calculated
 ...
 if(consistentScales==F){
  ...
  {
    count[[i]] <- get_modelcounts(control_table, treatment = unique(control_table$Treatment)[i], scales = myScales)
  }
...
public_count <- getMyCounts(mypublic_table[which(mypublic_table$Treatment != "Communal"),], consistentScales=T)
public_count[c(2,3,5,1,4),]
private_count <- getMyCounts(myprivate_table[which(myprivate_table$Treatment != "Communal"),], consistentScales=T)
private_count[c(5,2,4,1,3),]
# Table S6 (consistent scales to the level of filtered PA/sustuse)
# still, imbalance levels are VERY high in these datasets, and thus should be interpreted with caution
# this means we are left with many fewer altPA results 

altPA.public_count <- getMyCounts(mypublic_table_altPA[which(mypublic_table_altPA$Treatment != "Communal"),], consistentScales=T)
altPA.public_count[c(2,4,5,1,3),]
altPA.private_count <- getMyCounts(myprivate_table_altPA[which(myprivate_table_altPA$Treatment != "Communal"),], consistentScales=T)
altPA.private_count[c(2,4,5,1,3),]

# SI plots
# SI tables

# Table S7 (all scales)
public_count_S7 <- getMyCounts(mypublic_table, consistentScales=F)
public_count_S7[c(3,4,6,2,5,1),]
private_count_S7 <- getMyCounts(myprivate_table, consistentScales=F)
private_count_S7[c(6,3,5,2,4,1),]

altPA.public_count2 <- getMyCounts(mypublic_table_altPA, consistentScales=F)
altPA.public_count2[c(3,5,6,2,4,1),]
altPA.private_count2 <- getMyCounts(myprivate_table_altPA, consistentScales=F)
altPA.private_count2[c(3,5,6,2,4,1),]

# write with rearranged rows
setwd(wd_sum)
# table S5
write.csv(public_count[c(2,3,5,1,4),], "effectdirection_public.csv")
write.csv(private_count[c(5,2,4,1,3),], "effectdirection_private.csv")

# table S6
write.csv(altPA.public_count[c(2,4,5,1,3),], "effectdirection_public_altPA.csv")
write.csv(altPA.private_count[c(2,4,5,1,3),], "effectdirection_private_altPA.csv")
...
write.csv(altPA.public_count2[c(3,5,6,2,4,1),], "effectdirection_public_altPA_S7.csv")
write.csv(altPA.private_count2[c(3,5,6,2,4,1),], "effectdirection_private_altPA_S7.csv")
...
get_ranks <- function(control_table, control, consistentScales){
  ...
  if(consistentScales==T){
    ...
    # rendering this a non-consistent scale. 
    mydata$rm <- NA
    mydata$rm <- ifelse(is.na(mydata$p_value),T, F)# specify the if condition to be sensitive to p-values being NA - i.e. a model that wasn't calculated
  ...
  for(i in 1:length(mydata))
  {
    ...
    mydata[[i]]$rank <- rank(mydata[[i]]$Effect, na.last="keep")
...
vspublic_ranks <- get_ranks(mypublic_table[which(mypublic_table$Treatment != "Communal"),], control="public", consistentScales = T) 
vspublic_ranks[c(2,3,5,1,4),]
vsprivate_ranks <- get_ranks(myprivate_table[which(myprivate_table$Treatment != "Communal"),], control="private", consistentScales = T)
vsprivate_ranks[c(5,2,4,1,3),]

#version table s6
altPA.vspublic_ranks <- get_ranks(mypublic_table_altPA[which(mypublic_table_altPA$Treatment != "Communal"),], control="public", consistentScales = T) 
altPA.vspublic_ranks[c(2,3,5,1,4),]
altPA.vsprivate_ranks <- get_ranks(myprivate_table_altPA[which(myprivate_table_altPA$Treatment != "Communal"),], control="private", consistentScales = T)
