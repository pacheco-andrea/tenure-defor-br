...
# regression analysis 
# ****************************** #
# this script runs GLMS for all our spatiotemporal scales - filtered by existing PAs
...
# job:
setwd(paste0(wd_main, "inputs/01_matched/forStatsAnalysis_PA"))
input <- list.files()
i=as.integer(Sys.getenv('SGE_TASK_ID')) # 174
...
if(nrow(data) > 40){ # if the data frame has more than 40 observations,
  ...
  setwd(paste0(wd_main, "/outputs/GLMs_noTL_clusterMun_PA/"))
  ...
  ame$n <- nrow(data)
  setwd(paste0(wd_main, "/outputs/AMEs_noTL_clusterMun_PA/"))
  write.csv(ame, paste0(n, "_ame", ".csv"))
}

if(!exists("myglm")){
  setwd(paste0(wd_main, "/outputs/noTL_clusterMun_errors_PA/"))
  if(nrow(data) <= 40){
