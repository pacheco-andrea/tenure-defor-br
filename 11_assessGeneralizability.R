...
# set up job
setwd(paste0(wdmain, "/outputs/toAssessGeneralizability_2"))
input <- list.files()
...
if(class(assess_object2) == "generalize_assess"){
  ...
  # write out covariate table with asmd scores 
  asmd <- assess_object2$covariate_table
  asmd$stScale <- name
  setwd(paste0(wdmain, "/outputs/asmd_tables"))
  write.csv(asmd, paste0(name, ".csv"))
  
  g$g_indexF <- assess_object2$g_index
}
setwd(paste0(wdmain, "/outputs/g_index"))
...
setwd(paste0(wdmain, "/outputs/trim_pop"))
