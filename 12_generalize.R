...
setwd(paste0(wdmain, "/outputs/toTATE"))
input <- list.files()
...
# # (MATCHED) SAMPLE AVERAGE TREATMENT EFFECTS (SATE) are our AME's i've calculated in previous version of this analysis
...
if(nrow(data) <= 40){
  setwd(paste0(wdmain, "/outputs/generalize_losses"))
...
if(nrow(data) > 40 & sum(data[,treatment]) > 0){ # if the data frame has more than 40 observations,
  ...
  setwd(paste0(wdmain, "/outputs/generalize_glms"))
  ...
  setwd(paste0(wdmain, "/outputs/generalize_ames"))
  write.csv(ame[a,], paste0(name, ".csv"), row.names = F)
  setwd(paste0(wdmain, "/outputs/fullRegResults"))
  write.csv(ame, paste0(name, ".csv"), row.names = F)
}
